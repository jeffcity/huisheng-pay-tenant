...
- 新结构：React + Arco Design 壳（与平台端同款），4 个内页以真实 HTML 文件存放，经 embed 管线注入 iframe srcdoc。
- 内页源：`public/legacy/sources/{overview,merchants,system,login}.html`（从旧单体 base64 逐字节解码，roundtrip 校验通过）。
